...
internal partial class Manager : Form {
  ...
  public Manager(long gameId, API.Client client, bool isAuto = false) {
    ...
    this._StatisticsDataGridView.Columns[1].ReadOnly =
    ...
    string name =
        this._SteamClient.SteamApps001.GetAppData((uint)this._GameId, "name");
    if (name != null) {
      base.Text += " | " + name;
    } else {
    ...
    // this.UserStatsStoredCallback = new API.Callback(1102, new
    // API.Callback.CallbackFunction(this.OnUserStatsStored));
  ...
  private void AddAchievementIcon(Stats.AchievementInfo info, Image icon) {
    if (icon == null) {
      ...
      this._AchievementImageList.Images.Add(
          info.IsAchieved == true ? info.IconNormal : info.IconLocked, icon);
    }
  }
  ...
  private void DownloadNextIcon() {
    ...
    this._DownloadStatusLabel.Text =
        $"Downloading {this._IconQueue.Count} icons...";
    ...
    this._IconDownloader.DownloadDataAsync(
        new Uri(_(
            $"https://cdn.steamstatic.com/steamcommunity/public/images/apps/{this._GameId}/{(info.IsAchieved == true ? info.IconNormal : info.IconLocked)}")),
        info);
  }
  ...
  private static string GetLocalizedString(KeyValue kv, string language,
                                           string defaultValue) {
  ...
  private bool LoadUserGameStatsSchema() {
    ...
    var stats =
        kv[this._GameId.ToString(CultureInfo.InvariantCulture)]["stats"];
    ...
    foreach (var stat in stats.Children) {
      ...
      var rawType = stat["type_int"].Valid ? stat["type_int"].AsInteger(0)
      ...
      case APITypes.UserStatType.Integer: {
        ...
        string name =
            GetLocalizedString(stat["display"]["name"], currentLanguage, id);
        ...
        if (stat.Children != null) {
          foreach (var bits in stat.Children.Where(
            ...
            foreach (var bit in bits.Children) {
              ...
              string name = GetLocalizedString(bit["display"]["name"],
                                               currentLanguage, id);
  ...
  private void OnUserStatsReceived(APITypes.UserStatsReceived param) {
    ...
    try {
      ...
      this.EnableInput();
      MessageBox.Show("Error when handling achievements retrieval:\n" + e,
    ...
    if (isAutomatic) {
      ...
      foreach (Stats.AchievementInfo info in achievements) {
        if (this._SteamClient.SteamUserStats.SetAchievement(
                info.Id, info.IsAchieved) == false) {
          Application.Exit();
        }
      }
  ...
  private void RefreshStats() {
    ...
    if (callHandle == API.CallHandle.Invalid) {
      MessageBox.Show(this, "Failed.", "Error", MessageBoxButtons.OK,
  ...
  private void GetAchievements() {
    ...
    foreach (var def in this._AchievementDefinitions) {
      ...
      if (this._SteamClient.SteamUserStats.GetAchievementAndUnlockTime(
              def.Id, out bool isAchieved, out var unlockTime) == false) {
        continue;
      }

      bool wanted =
          (wantLocked == false && wantUnlocked == false) || isAchieved switch {
            true => wantUnlocked,
            false => wantLocked,
          };
      if (wanted == false) {
      ...
      if (textSearch != null) {
        if (def.Name.IndexOf(textSearch, StringComparison.OrdinalIgnoreCase) <
                0 &&
            def.Description.IndexOf(textSearch,
      ...
      Stats.AchievementInfo info = new() {
        ...
        UnlockTime =
            isAchieved == true && unlockTime > 0
                ...
                : null,
        IconNormal =
            string.IsNullOrEmpty(def.IconNormal) ? null : def.IconNormal,
        IconLocked = string.IsNullOrEmpty(def.IconLocked) ? def.IconNormal
                                                          : def.IconLocked,
      ...
      ListViewItem item = new() {
        ...
        Text = info.Name,
        BackColor =
            (def.Permission & 3) == 0 ? Color.Black : Color.FromArgb(64, 0, 0),
      };
  ...
  private void GetStatistics() {
    ...
    foreach (var stat in this._StatDefinitions) {
      ...
      if (stat is Stats.IntegerStatDefinition intStat) {
        if (this._SteamClient.SteamUserStats.GetStatValue(
                intStat.Id, out int value) == false) {
          continue;
        }
        ...
        if (this._SteamClient.SteamUserStats.GetStatValue(
                floatStat.Id, out float value) == false) {
          continue;
        }
  ...
  private void AddAchievementToIconQueue(Stats.AchievementInfo info,
                                         bool startDownload) {
  ...
  private int StoreStatistics() {
    ...
    var statistics =
        this._Statistics.Where(stat => stat.IsModified == true).ToList();
    ...
    foreach (var stat in statistics) {
      if (stat is Stats.IntStatInfo intStat) {
        if (this._SteamClient.SteamUserStats.SetStatValue(
                intStat.Id, intStat.IntValue) == false) {
          MessageBox.Show(
              this,
        ...
        if (this._SteamClient.SteamUserStats.SetStatValue(
                floatStat.Id, floatStat.FloatValue) == false) {
          MessageBox.Show(
              this,
              $"An error occurred while setting the value for {stat.Id}, aborting store.",
              "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
  ...
  private void OnRefresh(object sender, EventArgs e) { this.RefreshStats(); }
  ...
  private bool Store() {
    if (this._SteamClient.SteamUserStats.StoreStats() == false) {
      MessageBox.Show(this, "An error occurred while storing, aborting.",
                      "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
      return false;
    }
  ...
  private void OnStore(object sender, EventArgs e) {
    ...
    MessageBox.Show(
        this, $"Stored {achievements} achievements and {stats} statistics.",
        "Information", MessageBoxButtons.OK, MessageBoxIcon.Information);
    this.RefreshStats();
  }

  private void OnStatDataError(object sender,
  ...
  private void OnResetAllStats(object sender, EventArgs e) {
    if (MessageBox.Show("Are you absolutely sure you want to reset stats?",
    ...
    bool achievementsToo =
        DialogResult.Yes ==
        MessageBox.Show("Do you want to reset achievements too?", "Question",
                        MessageBoxButtons.YesNo, MessageBoxIcon.Question);

    if (MessageBox.Show("Really really sure?", "Warning",
                        MessageBoxButtons.YesNo,
                        MessageBoxIcon.Error) == DialogResult.No) {
      return;
    }

    if (this._SteamClient.SteamUserStats.ResetAllStats(achievementsToo) ==
        false) {
      MessageBox.Show(this, "Failed.", "Error", MessageBoxButtons.OK,
                      MessageBoxIcon.Error);
      return;
  ...
  private void OnCheckAchievement(object sender, ItemCheckEventArgs e) {
    ...
    if (this._AchievementListView.Items[e.Index].Tag is not
            Stats.AchievementInfo info) {
      return;
    }
    ...
    if ((info.Permission & 3) != 0) {
      if (isAutomatic) {
        ...
        MessageBox.Show(this,
                        "Sorry, but this is a protected achievement and " +
                        "cannot be managed with Steam Achievement Manager.",
                        "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
